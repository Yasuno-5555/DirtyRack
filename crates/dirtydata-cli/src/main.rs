...
use dirtydata_intent::{IntentState, IntentStatus};
...
fn cmd_daemon() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Start Audio Engine
    let engine = dirtydata_runtime::AudioEngine::new(graph)?;
    ...
    loop {
        if engine.check_crash() {
            println!("{} PLUGIN CRASH DETECTED! Instantly falling back to Frozen Asset (Silence)", "⚡".red().bold());
        }
